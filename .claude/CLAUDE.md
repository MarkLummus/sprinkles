...
- **Impeccable and GSD in sync**: Impeccable evaluates and decides (`shape`, `critique`, `audit`, `document`, `init` write `.impeccable/`, `DESIGN.md`, `PRODUCT.md`); every edit under `app/` goes through a GSD command. Impeccable's refine, enhance and fix commands describe work for `/gsd-quick`, `/gsd-quick-batch` or a phase plan and never edit `app/` directly. The loop is critique → brief revision → GSD plan/execute → re-critique; a critique snapshot self-closes when its target file changes. The GSD ui-review step (`verify:post`, autonomous) dispatches `gsd-ui-review`, which is shadowed by a project skill (`.claude/skills/gsd-ui-review/`) that runs the Impeccable audit instead of the generic 6-pillar audit.
...
- Every `<a>` the app renders (each react-router `Link` and `NavLink`) carries `tabIndex={0}` — WebKit on Apple platforms lets Tab reach a link only under its TabsToLinks preference (off, with no iPadOS switch, untouched by Full Keyboard Access) or when the link sets an explicit tabindex, so an untagged link cannot be reached by keyboard on the iPad (G-03.4-r3-3, G-03.4-r4-1); each link-rendering component's own test pins it on rendered markup with an exact link count, except router.jsx's running head and RecipePage.jsx's not-found link, which the node harness cannot render and are pinned on source text in RecipePage.test.jsx.
<!-- GSD:conventions-end -->
