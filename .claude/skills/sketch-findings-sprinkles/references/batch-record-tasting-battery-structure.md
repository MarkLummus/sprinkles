...
- Footer: Cancel | Save batch | Add tasting. Empty status line. Undo hidden. **[Superseded by the
  ...
  are two of them — the end-of-record ceremony below the shared Next time and the foot ceremony. Lines 301–314.]**
...
- Helper `.helper` (line 169): grotesk; `--type-control` (13px); line-height 1.4; weight 400; `font-style: normal`; `text-transform: none`; `letter-spacing: 0`; `color: var(--ink)`. Used for "select all that apply" (line 265), the tasting status (line 244) and the record status (line 304).
...
**The box-or-word rubric (sketch 008 @ `0fa1015`, header line 230 and Settled values lines 268–282 (the recorded figure at 279 since `109733d`); DESIGN.md § The binder @ `a0126a2`):** which shape a control takes, now written down.
