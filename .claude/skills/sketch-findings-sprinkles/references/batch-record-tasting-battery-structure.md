...
- `.axis` (line 71): `margin: 0 0 var(--gap-m); min-width: 0`. `.axis .head` (lines 72, 155; base `.head` line 70, wrap line 115): `display: flex; align-items: center` (line 155 — revised 2026-09-14 from `baseline`, see the departures list); `gap: var(--gap-xs); margin-bottom: var(--gap-xs); min-height: 24px; flex-wrap: wrap`.
- Per-axis Clear `.axis .head .clear` (markup line 403, a `.text-control` hidden until the axis is marked; revised 2026-09-14): line 128 `margin-left: auto; margin-block: -4px` — right-aligned on the caption row, the same placement as Remove tasting on the TASTING row, with the `.text-control`'s 24px `min-height` (line 128) pulled to 16px in flow so the head row stays 24px whether or not Clear is showing. Below 760px, line 177 `margin-block: -10px` pulls the 44px touch box (line 177's `min-height: 44px`) to 24px in flow. In 3-column desktop mode, line 65 `[data-cols="3"] .axis:not(.axis--declared) { padding-right: var(--gap-m) }` keeps a right-aligned Clear 20px off the next column's caption and the vertical rule. Measured in a real tab at 1188px and 393px: picking a stop moves neither the caption, the head row (24px before and after) nor the stops; at 393px Clear's 44px box sits at x 330–361, clear of the stops (right edge 248).
...
- **Per-axis Clear placement (D-22 edit of 2026-09-14, after CONTEXT.md's "confirmed as-sketch" bullet; the HTML governs per D-01)** — Mark reported that the original Clear grew the head row from 24px to 44px below 760px and dropped the caption ~10px off its baseline; "inline with the stops" was measured and rejected (228.6px needed in a 213.3px column in 3-column mode). Four lines changed, numbers unchanged: line 65 appends `[data-cols="3"] .axis:not(.axis--declared) { padding-right: var(--gap-m); }`; line 128 appends `.axis .head .clear { margin-left: auto; margin-block: -4px; }`; line 155 `.axis .head` `align-items: baseline` → `center`; line 177 appends `.axis .head .clear { margin-block: -10px; }`. Clear is now right-aligned on the caption row with a 24px hit box (16px in flow) on desktop and a 44px hit box (24px in flow) below 760px.
