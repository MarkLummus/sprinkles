...
- Inline state next to the axis name: `(3)` in pen blue once marked, `(Not recorded)` in muted italic when not. A per-axis Clear text-control — the only way back: picking is final, and clicking an already-picked stop changes nothing (Mark, 2026-09-16; the earlier click-again-clears reading is struck).
...
- **Bitter is not a goldilocks axis** — "too little bitterness" carries no information. It's a presence/severity toggle reading the plain word `Bitter`, with no suffix and no `aria-label` of its own: it sits in the defects' "This recipe only" group, whose cue carries the meaning (ninth round, 2026-09-14). The earlier "Bitter · declared" form, with `aria-label="Declared for this recipe: Bitter"` trailing the universal defects row after a wider gap, is retired — in the sketch, in the app, and in the saved read view (2026-09-17).
