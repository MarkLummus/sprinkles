...
**The ceremony.** "Record a batch" is a control in the margin, in the binder's button style. It opens the pen layer: the as-made column appears, each step gains its strike and line, the churn section appears with the churn date first, and focus lands on the date. Everything the maker types is pen blue inside a hairline ink field. "Save batch" is explicit. Saving snapshots the version's rows and coefficients (BATCH2-01), returns the page to reading, and leaves the record in place: still blue, because it is the record, but now text with no field around it. Form carries state; colour never changes on save. The version line under the recipe name then carries the latest churn date, never the print date and never a count (Phase 1 D-13). "Add a tasting" is the same ceremony on a smaller field: a dated tasting section opens in the margin, is written, and is saved.
...
- **Hierarchy while recording:** churn date; the as-made column; the steps' strikes and lines; the churn section; the tasting section if one is being written; save. Reading order is the sheet's page order, and the tab order follows it: down the as-made column, then the steps top to bottom, then the churn section top to bottom, then each tasting top to bottom, then save.
