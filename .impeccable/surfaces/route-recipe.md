...
- **The imprint** (revised 2026-09-08 by Mark after the whole-page critique; a swap of the margin above the formulation note on a churned version was considered the same day and rejected, because it still left the record's controls inside the printed spread). A book's front matter carries its edition and printing dates on one page before the text begins, and this page does the same. The headnote no longer spans the page: it takes column one of the top band, name, version line and prose at the measure, and beside it in column two — the white space the critique found to the right of the headnote — sits the imprint, a region wearing its own running head. The imprint holds everything about the recipe that is not the recipe: the version strip; the lineage line with its show-changes toggle; the churned date and, when there is more than one, the batch list; and every opener — "Develop the next version", "Record a batch", "Amend", "Add a tasting". When a pen is open, its ceremony lives here too: the version line, reason, citation and saves for the plan's pen; the churn date, save and cancel for the batch's; the tasting date, save and cancel for a tasting's. The spread beneath the band — table and method in column one, formulation note then margin in column two — is then the printed page and nothing else: while a pen is open it carries in-place ink (grams fields, the as-made column, a step's skipped mark and line, prose paragraphs being written, the churn and tasting fields where the record reads), never a button. The margin keeps the record's content in the sheet's order — measured values, draw notes, tastings, then advisories, then authored notes — and no control. Its plain-language name is "Imprint" in this brief; whether the running head says that or "Versions and batches" is settled in GSD discussion, as "Develop the next version" was. Column two's order beneath the band is unchanged: the formulation note beside the table, the margin after it. While a pen is open, and only then, its save and cancel pair is repeated once at the foot of the page — a full-width band after both columns, the page's end matter to the imprint's front matter (Mark, 2026-09-08) — so the tab order arrives at Save after the last field and a maker at the top can save without scrolling. Only the pair repeats: no opener, no field, no running head of its own; the blocked-save sentence shows beside both pairs, since it is one state of the page.
...
- **Hierarchy:** name and version line; the imprint; ingredient table; formulation note; method; margin (record, advisories, authored notes). The top band is headnote left and imprint right (§ 3, revised 2026-09-08); beneath it column one is the recipe as written (table, then method) and column two is what the sheet does not print (formulation note, then margin). Reading order and tab order run headnote, imprint, table, method, formulation note, margin, and, while a pen is open, the repeated save and cancel pair at the foot of the page; the imprint's ceremony comes first in that order and the foot's pair last, so Save is one Tab from the last field and one click from the top.
- **Controls, the binder** (revised 2026-09-08; `DESIGN.md` § Buttons said no treatment existed and the critique found the browser's own chrome painting a fifth colour on every button, checkbox, focus ring and date field). Every control is drawn in ink at hairline weight with no fill, no radius and no icon, its label in the grotesk. A disabled control keeps its label and changes only its stroke, from solid to dashed, with its reason stated in words beside it, never dimmed to grey. A pressed or current state (the show-changes toggle, the current version in the strip, the open batch in its list, a marked row) is weight plus an outline at graduation weight; keyboard focus is an outline at baseline weight, offset the same, on every focusable element — heavier than state, so a focused pressed control reads as both, in place, with no colour. A checkbox is drawn as the marks control's stops already are: an ink square, filled ink when checked. A date field stays the browser's own date input for its validation and keyboard entry, with its calendar icon hidden; the browser's segment highlight while a part is selected is a named exception to the four-colour system (Mark, 2026-09-08). A prose field grows with its text and carries no resize grip. Adjacent controls are separated by the small gap, never by a space character. Every link on the page, the batch list included, is ink.
