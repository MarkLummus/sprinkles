...
- **Topology:** one page per recipe, no tabs and no mode toggle; the later-versions and later-batches lists open in place beneath their own row; the overlay for comparison; the browser's print for the sheet. Ingredient management and import are pages of their own, in the same material — not spreads, not trays.
