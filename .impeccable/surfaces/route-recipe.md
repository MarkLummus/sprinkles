...
- **The front matter, two rows** (revised 2026-09-09 by Mark from sketch 003 variant B; supersedes the imprint of 2026-09-08 and the tray of sketch 001, both retired). A book's front matter carries its edition and its printings before the text begins, and this page does the same — but across the page, not beside the headnote. Two outer rows span both columns above the spread. The **version row** carries the recipe name, the version line and the headnote prose at the measure on the left, and on the right the version's own metadata (From version, Why, From batch) with its acts: Next version, Show changes, and the count that opens the later-versions list beneath the row. The **batch row** carries the churn line and the maker's own words on the left, and the measured values as labelled cells at figure size on the right — Out of the machine, Air, Melt test, Time to temperature — with tasting marks as a second cell group and its acts: Record another, Add tasting, Correct, and the count that opens the later-batches list beneath the row. The two rows stay two because their acts differ: Next version and Record another act on the plan, Correct and Add tasting act on one churn. Putting them on one row is how the imprint got overloaded. A rule at baseline weight closes the front matter above Ingredients. The rows wear no running head (Mark, 2026-09-09): they are front matter, not regions of the recipe, and the version's name and the word `churned` identify them; bookcloth resumes at Ingredients. The **margin is retired as a region** — its content splits, the record upward into the batch row, Things to check and the carried-forward Notes downward under Balance in column two. Column two now holds Balance level with Ingredients, then Things to check, then Notes, and nothing sits above Balance.
- **The tip, and the lists.** The page holds the tip: the latest version and its latest batch. Earlier ones are one control away, opening as a list beneath the row they belong to and never as a second page or a tray. This follows the binder — 29 sheets, one line of work — and Mark's own account of how he works. Version and batch stay separate records (D04): the same plan churned twice is two batches of one version. The words merge where the maker reads them: a version with no batch reads `not yet churned` (`draft` is acceptable), one with a batch `churned 2 Aug 2026`, a repeated one `churned twice`. Absence is stated by the guide's "Saying absence" rule.
...
| Entries in an opened later-versions list | 0–5; three across at 1440 |
...
- **Topology:** one page per recipe, no tabs and no mode toggle; the versions and batches lists open in place beneath their own row; the overlay for comparison; the browser's print for the sheet. Ingredient management and import are pages of their own, in the same material — not spreads, not trays.
- **History controls name a whole set, never a direction** (Mark, 2026-09-17; `/impeccable clarify`). The two disclosures read **"Versions (n)"** on the version row and **"Batches (n)"** on the batch row, and each count is the *complete* set: every version of this recipe, and every batch of the version in view, each including the one being read. "Later" is struck everywhere it appeared — as the lineage label, as the control's words ("3 later versions"), and as the disclosed region's own name.

  The old wording was relative to wherever the maker happened to be standing: "3 later versions" named a different three depending on which version was open, and a maker on version 1 of four was told about three while a maker on version 3 was told about one, with no way to see the set. Worse, it described the subtree below the current version only — so an ancestor or a sibling was unreachable from a page that nonetheless claimed to count the versions. This brief's own raise from the round already asked for the opposite: "every superseded version reachable in one strip" (§ 3). A parenthetical count is also the one form that needs no plural: "Versions (1)" and "Versions (4)" are the same sentence, which matters for a label that must translate.

  Inside each list, position is named rather than implied: the entry the page is showing wears **"In view"**, and the newest version of the recipe wears **"Latest"**. Both can sit on the same entry, and both are words, not a colour or a weight — form carries state (`DESIGN.md`), and a marker that exists only as a style is invisible to a screen reader and to forced colours. Every other entry wears nothing, so absence stays legible.

  "Next version" and "Record another" are untouched: they are acts, not history, and they already say what they do.
...
- What an opened later-versions list does past four entries; three across at 1440 worked.
...
- ~~Whether the version strip shows the batch count per version~~ — retired with the strip: the version row carries the later-versions count and the batch row carries the churn.
