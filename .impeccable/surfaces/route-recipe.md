...
- **Sheet**, per version: Sheet title, Sheet description, ingredients by portion, Method, Notes. Next version copies the parent's Sheet title and Sheet description.
- **Why the two names** (Mark, 2026-09-23). The Recipe name is what a maker picks in a list; it stays put. The Sheet title and Sheet description are the visible name and introduction, tweaked version by version as the recipe is perfected. The printed sheet, and the Sheet on screen, show the Sheet title.
- **The Sheet has two forms.**
  - **Notebook form.** Printed: blank as-made fields and the blank batch and tasting log pages (Phase 4, `route-print-recipe-sheet.md`). On screen: the batch in view's as-made column and its Method changes — strikes, skips, edited step text — in pen blue against the black plan. Balance and Things to check belong to this form, on screen only.
...
**6. Open, not for a builder to invent.** The rename label; where the Batches (n) chooser sits (the canvas settles it); the desktop layout (A, B or C).
