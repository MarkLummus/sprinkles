...
| Two contexts, Sheet and App, with prefixed tokens (`sheet-*` / `app-*`); the paper world is confined to the spread, the batch log and the print sheet, and everything outside is the App context with the D17 destination palette | Mark's 2026-09-21 design approval; one token per value app-wide, split only for a named exception | ✓ Good — Phase 03.4 |
| The hand is Caveat in sheet pen blue, for observations and ideas only, never specs or app text; served from a vendored woff2 with recorded provenance, forced-colours and print fallbacks | Approved 2026-09-21; TRUST-01 keeps every asset first-party | ✓ Good — Phase 03.4 |
| Home reads "Active work first": a lead card for the recipe with the latest event, then rows in event order; a back-dated kitchen date never moves a recipe's place | route.md; lastEventAt reads recorded and created dates only | ✓ Good — confirmed in Phase 03.4 UAT |
| Device UAT is served from the production build (`build` then `preview --host`), never the dev server | The iPad's "seconds to load" was 6 MiB of unbundled dev modules over the LAN; a device measurement against the dev server means nothing | ✓ Good — Phase 03.4 |
| Every `<a>` the app renders carries an explicit `tabindex="0"`, pinned on rendered markup per component | WebKit's TabsToLinks gate skips links on Apple platforms and Full Keyboard Access does not widen it; confirmed on the iPad in UAT rounds four and five | ✓ Good — Phase 03.4 |
| Full Keyboard Access stays off on the iPad; keyboard UAT is tap-then-Tab with it off | With it on the hardware keyboard locked until a power cycle, twice, with focus never in the page; the app has no code path that could cause it | ✓ Good — Mark, 2026-09-22 |
| The same tabindex rule for every button, checkbox and radio (38 sites) is deferred to a shared Button component rather than a gap-closure plan | Mark, 2026-09-22: "defer this for now. I like the shared component idea the best, since we are using react"; the recipe page's controls are unreachable by Tab on iPadOS until it lands | ⚠️ Revisit — todo 2026-09-22-shared-button-component-carries-tabindex-for-webkit |
...
---
*Last updated: 2026-09-22 after Phase 03.4*
