...
- [ ] **BATCH1-02**: Maker can record measured values (come-up time, draw temperature, overrun, meltdown); a field left blank stays unknown and is never filled from the recipe.
- [ ] **BATCH2-01**: A batch stores a snapshot of the recipe rows and the ingredient coefficients it was computed with; later edits to the recipe or ingredient data do not change what the batch shows.
