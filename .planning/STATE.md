...
Last activity: 2026-09-18 - Completed quick task 260917-wrd: one version identity, wherever a version is named
...
| 260916-ch0 | Apply .impeccable/fix/2026-09-16__one-fill-for-every-on-state.md — one fill for every on state: Show changes becomes the square and the word, the checkbox fills pen blue instead of ink, both take the 44px touch target and join the forced-colors block. Retires the last bold-plus-outline pressed state (sketch 008 lines 109, 230, 373). UAT 5/5: 32px fine / 44px coarse measured in real engines; pressed square fills pen blue with no bold and no outline; forced colours intact. UAT #3 surfaced a pre-existing, unrelated bug, parked at .planning/debug/show-changes-hides-pen.md. | 2026-09-16 | 1bcd9de, 57a6b2f, 6a8b44a, f79791c | [260916-ch0-apply-impeccable-fix-2026-09-16-one-fill](./quick/260916-ch0-apply-impeccable-fix-2026-09-16-one-fill/) |
| 260915-x6n | Implement sketch 009's settled wide-touch decisions — decision C (the stop grows in height only) and M4 (the melt row's equal caption-to-control gaps), plus sketch 008's approved 16px touch font bump | 2026-09-15 | 246f172 | [260915-x6n-implement-sketch-009-s-settled-wide-touc](./quick/260915-x6n-implement-sketch-009-s-settled-wide-touc/) |
| 260916-xbh | Harden the batch record's save path, part 1 of 2 (critique fix 2): the churn date is named required before the refusal, and the blocked sentence renders once inside the date's own label instead of twice off-screen at the two ceremonies. Mark split fix 1 out on 2026-09-17 — the save's confirmation and its focus landing are page-scoped furniture and stay with the pending todo and the page shell's own Impeccable brief. | 2026-09-16 | e394209 | [260916-xbh-harden-the-batch-record-s-save-path-conf](./quick/260916-xbh-harden-the-batch-record-s-save-path-conf/) |
| 260917-e5k | Retire three hint rules nothing renders (`.pen-hint`, `.batch-margin__hint`, `.versions__hint`) and retarget the two cross-cutting tests that pinned them at the four live helper/status rules; move the version pen's refusal and its four storage-failure paths off the page channel into the pen's own form-scoped region, leaving the page region to speak only on a successful save. Browser-verified. | 2026-09-17 | 51c862e, 1c50cb7 | [260917-e5k-retire-the-orphaned-hint-rules-and-move-](./quick/260917-e5k-retire-the-orphaned-hint-rules-and-move-/) |
| 260917-ewf | Anchor the page-scoped save notice beneath the running head instead of the viewport corner, and open the project's first print layer by keeping it off the sheet. The running head lifts into the routed shell (above the keyed page) so the notice can anchor to it and still survive the save's navigation; the notice stays out of flow, so nothing reflows. Browser-verified: 0px reflow, aligned left edges, 6px of air under the head. | 2026-09-17 | f0a12ce, 8db14cc, d2f075b, 0dd391e | [260917-ewf-anchor-the-page-notice-beneath-the-runni](./quick/260917-ewf-anchor-the-page-notice-beneath-the-runni/) |
| 260917-gjo | One shared page gutter: `--gap-page` is defined in tokens.css and stepped to the narrow value on `:root` inside app.css's existing 600px block, so the running head, the save notice, the page shell and the not-found body read one value instead of four. Closes the sub-600px divergence where the head and notice hung 28px right of the content. Browser-verified at 1920 and at 393. | 2026-09-17 | 4bcac2f | [260917-gjo-one-shared-page-gutter-for-the-running-h](./quick/260917-gjo-one-shared-page-gutter-for-the-running-h/) |
| 260917-h83 | The recipe list route joins the shared page gutter: a `.list-page` wrapper reads `--gap-page` as the fifth consumer, so the app's entry screen is framed like every other route instead of sitting flush at 0. Browser-verified at 1920 and 393; the shared-gutter contract and its no-media guard widened from four boxes to five, plus an orphan guard pinning that the class is actually rendered. | 2026-09-17 | e568e09 | [260917-h83-the-recipe-list-route-takes-the-shared-p](./quick/260917-h83-the-recipe-list-route-takes-the-shared-p/) |
| 260916-0d4 | SR-6: the churn date takes the author box model on WebKit — appearance:none, so .ink-field applies at all; root cause established on Mark's iPad and confirmed in the shipped app | 2026-09-16 | 1ee6c37 | [260916-0d4-sr-6-the-date-input-takes-the-author-box](./quick/260916-0d4-sr-6-the-date-input-takes-the-author-box/) |
| 260917-odu | History controls name a whole set, never a direction: both disclosures now count and reveal the COMPLETE set. The version row reads `Versions (n)` over every version of the recipe (the one in view included, an ancestor and a sibling both reachable) in place of the subtree walk behind "n later versions"; the batch row reads `Batches (n)` over every batch of the version in view, so a version with one batch no longer announces none. Position inside each list is named in words — `In view` on the entry being read, `Latest` on the newest (ordered[0], positional, so marker and order cannot disagree), both on one entry where they coincide. The entry in view is no longer a link to itself and has no `Open`. The struck `Later` dt had no replacement word, so the control left the dl for its own line below it (Mark's call, 2026-09-17) — which is also what keeps it available while the batch pen is open (D-UAT-2). Fixed a latent defect the complete set would have exposed: a root version's card read `from null`. Browser-verified at 1024 and 393 on a four-version, two-branch store. | 2026-09-17 | bb4304b, b7fd19b | [260917-odu-history-controls-name-a-whole-set-versio](./quick/260917-odu-history-controls-name-a-whole-set-versio/) |
| 260917-vev | The two history panels become one ruled register: both disclosed lists retire the card grid and the wrapping flex row for a single stacked list at the reading measure, capped at `--measure-prose`. Each row carries identity and its state words on the left with provenance beneath, and written date over batch history right-aligned and tabular; a rule closes every row including the last. The version title is the only link — `Open` is struck everywhere and the row in view is not a link to itself. Provenance is one line with the citation winning (`after the batch of ...` over `from ...`), and a root version renders neither while keeping its height from the record column. A version's batch history is stated in words by a new framework-free `batchHistoryWords` in `domain/batch.js` — `not yet churned` / `churned 2 Aug 2026` / `churned twice - last ...` / `churned 3 times - last ...`. Browser-verified on a four-version, two-branch store at 1920, 768 and 393: the panel measures 542px (65ch), not the 1824px six-column grid it replaced; all four record blocks share one right edge; zero horizontal overflow at 393, where the record block wraps beneath the identity rather than crushing it. | 2026-09-17 | 5600936, ef13421, b384f4b | [260917-vev-apply-impeccable-fix-2026-09-17-history-](./quick/260917-vev-apply-impeccable-fix-2026-09-17-history-/) |
| 260917-wrd | One version identity, wherever a version is named: a version now reads `Version 3 · 40 g oil · 800 g` on its own row and `Version 3 · 40 g oil · 800 g · In view` in the history register — a derived ordinal, the authored name, then the state words, from one `lineage.js` function returning the whole string so the two sites cannot drift. The ordinal is flat creation order read off `sortedVersions`'s own descending array reversed (`length - index`), never a second sort: it counts printings, not depth, so siblings take consecutive numbers and the provenance line beneath keeps answering parentage. The identity line became the version row's `h2` and the bare `Version` heading retired (a heading reading Version above a line reading Version 3 stutters); the authored name left the headnote rather than print twice, taking D-27's fork-landing focus with it — the `aria-label` override is gone because the visible line now says what it said. Browser-verified on a five-version, two-branch store at 1920 and 393: the register reads Version 5/4/3/2/1 with no gaps, a sibling cites its own parent and not its predecessor, a fork lands focus on the child's identity heading (`Version 5 · 35 g oil · 800 g · Latest`, accessible name equal to visible text), Save over renumbers nothing, and 393 has zero overflow with the longer identities wrapping rather than crushing. | 2026-09-18 | 2da6322, 25c232c, 8ce66d9 | [260917-wrd-apply-impeccable-fix-2026-09-18-one-vers](./quick/260917-wrd-apply-impeccable-fix-2026-09-18-one-vers/) |
