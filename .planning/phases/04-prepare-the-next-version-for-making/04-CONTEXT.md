...
- **D-00 (Mark, 2026-09-23, overrides the canvas drawings below):** The printed sheet **starts from the recipe page as built in the app**, together with the brief. The print view is that page's own component tree rendered for paper (controls, the Balance column and other screen furniture dropped, blanks added where the brief puts them), not a separately designed document. The canvas pages drawn on 2026-09-22/23 were designed from the brief's prose and differ from the built page; they are **not** the acceptance target. Any canvas or snapshot for Phase 4 must be derived from the rendered built page first.
- **D-00a (Mark, 2026-09-23):** The words **"formula" and "formulation" never appear in the app's visible text**, including sheet headings. Known occurrences today: the seeded headnote ("Scaled 0.8× from the 1 kg formula…", `app/src/data/olive-oil.js`) and an ingredient note in `app/src/data/library.js`; component and CSS names (`FormulationNote`, `.formulation-note`) are not visible and are a separate, optional rename.
