...
Three weights, three meanings, and they are provably ordered: **1px** is the outline on content the page is pointing at (a marked table row, the open batch, the current version, a pressed Show changes); **2px** is focus, and nothing else; the picked control's **fill** is a different sign entirely, so a picked thing and a focused thing never read as each other.
...
- **Checkbox:** a 13px ink square, filled solid ink when checked; its label names it ("Step 3, skipped").
