...
**Change from previous:** reverts to v1.0's sugars (sucrose 39, dextrose 175) while keeping v1.1's serving spec. PAC is wildly out of range and the page shows 0 % errors anyway.

**Zero handwriting. No process, no outcome.**

**Reconstructable?** Formula yes; **intent no.** This batch was churned. It reverts sucrose 88→39 and dextrose 128→175 — undoing the exact swap that v1.0 prescribed ("Less Dextrose, more Sucrose / Keep PAC, ↓POD") and v1.1 confirmed ("Good Texture / Scoopable / Good Sweetness"). PAC lands at **347 against a 220–230 target** — the largest spec violation in the binder — and the error row still reads 0 % across the board because the app only errors against the *recipe's own* targets, not the type's range.

So: a churned batch that walked back a proven improvement and blew the PAC target by 50 %, with no note explaining why and no record of how it tasted. Either it was a deliberate test of whether the sugar swap actually mattered — in which case the answer is lost — or it was a mistake. **Nothing on the page distinguishes a deliberate regression test from an error**, and that distinction is the whole value of the batch.
