...
Confirmed by Mark on 2026-09-18 (D13 authority, first confirmation): the paper world is binding inside three frames only — the recipe spread on screen, the batch and tasting log (black, blue, and a highlighter still to be drawn), and the printed sheet. Outside those frames the app uses a colorful context and navigation layer: a fixed destination palette for the app's own marks and guidance. The 2026-09-21 palette approval supersedes recipe-specific identity colours: use Option B with the revised Ingredients green and the approved app neutrals, specified in DESIGN.md. Shared interaction and feedback treatments remain unresolved. Colour never carries a verdict anywhere (D06). Direction contract: `.impeccable/surfaces/route.md`.
