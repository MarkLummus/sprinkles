...
// single source of truth. After the first write, the seeded recipe record,
// its version and its working-case batch are data like any other — saved
// through the same calls every later write uses.
export async function seedIfEmpty(repository) {
  const existing = await repository.listVersions();
  if (existing.length > 0) return;
  await repository.saveRecipe(oliveOilRecipe);
