...
3. Assets and processes stay distinguishable, but not necessarily separate. A process's evidence may be shown in the context of the asset it concerns when that aids understanding, and stays marked as the process's own. The batch in view's as-made grams and Instructions changes sit on the Sheet in pen blue against the black plan, while the rest of the batch record is App context. In print, a process can appear as blank space for the pen (DESIGN.md Context Boundary Rule).
